...
/**
 * Toggle a form's input to disabled to enabled
 */
assessToggleDisable = {
    /**
     * Current state
    ...
    disable: function(elementid) {
        ...
        this.state[elementid] = 'disabled';
    },

    /**
     * Enable all form elements, on first execution
     * returns false to allow form editing.  On second
     * execution, returns true
     */
    enable: function(button, submitString) {
        var form      = findParentNode(button, 'FORM', false, false);
        var elementid = form.getAttributeNode('id').value;

        if (this.state[elementid] == 'disabled') {
            ...
            // Convert submit button strings to proper values
            var elements = document.getElementById(elementid).getElementsByTagName('input');
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].type == 'submit') {
                    elements[i].value = submitString;
                }
            }

            // First time we enable, so don't submit form
            this.state[elementid] = 'enabled';
            return false;
        } else {
            // Second time around, let the form submit
            return true;
        }
    }
};
 ...
 * sections in the saved searches
 * box.
 **/
modContent.Search = {
    /**
     * Keeps track of all known search section IDs
     **/
    ids: new Array(),

    /**
     * Performs the onclick toggle.  Hides
     * all other search sections if any are
     * open and then toggles the one that
     * was clicked.
     *
     ...
     * @return void
     **/
    toggle: function(el, strShow, strHide) {
        for (var i = 0; i < this.ids.length; i++) {
            var child = document.getElementById(this.ids[i]);
            if (child.id == el.id) {
                continue;
            }
            if (!this.isHidden(child)) {
                this.doToggle(child, true, strShow, strHide);
            }
        }
        this.doToggle(el, true, strShow, strHide);
    },
     ...
     * @return void
     **/
    doToggle: function(el, persist, strShow, strHide) {
     ...
     * search section
     *
     * Also used to register all our section ids
     *
    ...
    cookieShow: function(id, strShow, strHide) {
        this.ids.push(id); // Registered

        var cook = new cookie('show:' + id).read();
        if (cook == null) {
            this.doToggle(document.getElementById(id), false, strShow, strHide);
        }
    },
 ...
 * CRS Search Form
 **/
modContent.SearchForm = {
    /**
     * Detects when the Return key is
     * pressed and then clicks the submit
     * button.  This is for IE which will
     * auto-click the last button (Save as),
     * which we don't want
     *
     * @param object e Event object
     **/
 ...
 * CRS Backup Export
 *
 * Consists of a series of dialog
 * boxes to talk a user through
 * exporting CRS content items
 * that were found during a course
 * backup
 **/
modContent.BackupExport = {
    /**
     * Panel displaying a "loading" image
     **/
    exportPanel: false,
     ...
     *
     * @param object strings Strings from PHP that are displayed to the user
     **/
    init: function(strings) {
        modContent.BackupExport.str = strings

        // Add class to skinning the dialog boxes
        YAHOO.util.Dom.addClass(document.body, 'yui-skin-sam');

        // Dialog button handlers
        var handleYes = function() {
            this.hide();
            modContent.BackupExport.exportContentItems();
        };
        var handleNo = function() {
            this.hide();
        };

        // Dialog to prompt user to perform export or not
        exportDialog = new YAHOO.widget.SimpleDialog("export", {
            width: "300px",
            zIndex: 20000,
            fixedcenter: true,
            visible: false,
            draggable: false,
            close: true,
            text: modContent.BackupExport.str.exportBody,
            icon: YAHOO.widget.SimpleDialog.ICON_HELP,
            constraintoviewport: true,
            modal: true,
            buttons: [ { text:modContent.BackupExport.str.yes, handler:handleYes, isDefault:true },
                       { text:modContent.BackupExport.str.no,  handler:handleNo } ]
        });

        exportDialog.setHeader(modContent.BackupExport.str.exportHeader);
        exportDialog.render(document.body);
        exportDialog.show();
    },
     ...
     * Runs the export
     **/
    exportContentItems: function() {
        modContent.BackupExport.exportPanel = new YAHOO.widget.Panel("wait", {
            width: "300px",
            fixedcenter: true,
            close: false,
            draggable: false,
        ...
        modContent.BackupExport.exportPanel.setHeader(modContent.BackupExport.str.waitHeader);
        modContent.BackupExport.exportPanel.setBody(modContent.BackupExport.str.waitBody);
        modContent.BackupExport.exportPanel.render(document.body);
        modContent.BackupExport.exportPanel.show();

        // Connection callbacks
        var callback = {
            success: modContent.BackupExport.exportSuccess,
            failure: modContent.BackupExport.exportFailure,
            scope: modContent.BackupExport
        }

        // Perform the export
        var conn = YAHOO.util.Connect.asyncRequest("POST", modContent.BackupExport.str.connUrl,
                                                   callback, "sesskey="+modContent.BackupExport.str.sesskey);
    },

    /**
     * Handles successful connections
     **/
    exportSuccess: function(o) {
        modContent.BackupExport.exportPanel.hide();
        modContent.BackupExport.displayResultDialog(
            modContent.BackupExport.str.successHeader,
            o.responseText,
     ...
     **/
    exportFailure: function(o) {
        modContent.BackupExport.exportPanel.hide();
        modContent.BackupExport.displayResultDialog(
            modContent.BackupExport.str.failHeader,
            modContent.BackupExport.str.failBody,
            YAHOO.widget.SimpleDialog.ICON_WARN
        );
    },

    /**
     * Displays a dialog with connection results
     **/
    displayResultDialog: function(headerText, bodyText, yuiIcon) {
        var handleClose = function() {
            this.hide();
        };

        resultDialog = new YAHOO.widget.SimpleDialog("result", {
            width: "400px",
            fixedcenter: true,
            zindex: 2000,
            visible: false,
            ...
            constraintoviewport: true,
            modal: true,
            buttons: [ { text:modContent.BackupExport.str.close, handler:handleClose, isDefault:true } ]
        });

        resultDialog.setHeader(headerText);
        resultDialog.render(document.body);
        resultDialog.show();
    }
};
 ...
 * @return bool
 **/
function change_parent_status(requestid, status) {
    if (window.opener.document.getElementById(requestid).value = status) {
        return true;
    }
    return false;
}
 ...
 *
 **/
function mod_enrolrequest_submit(url, formid) {
...
// Var to pass around the postid between functions
var mhpostid    = null;
var mhflagtype  = null;

/**
 * Creates the ajax xmlhttp object to open a live ajax connection
 *
 * @param integer linkid  ID of the selected post
 * @param string  type    Type of flag being toggled
 **/
function mod_hsuforum_toggleFlag(linkid, type) {
    xmlHttp = mod_hsuforum_getXmlHttpObject();
    if (xmlHttp == null) {
    ...
    mhpostid    = linkid;
    mhflagtype  = type;
    var url = 'toggleflag_ajax.php?postid='+linkid+'&type='+type;
    
    xmlHttp.onreadystatechange = mod_hsuforum_changeFlagIcon;
    xmlHttp.open("GET", url, true);
    xmlHttp.send(null);
}

/**
 * Callback function which takes the response from the xmlhttp request
 * and,  if successful, will replace image with alternate image.
 *
 **/
function mod_hsuforum_changeFlagIcon() {
    if (xmlHttp.readyState == 4) {
        // No action if result failed (database not changed)
        if (xmlHttp.responseText != '') {
            retval = xmlHttp.responseText;
            // Updates the title property
            updateLink = document.getElementById(mhflagtype+'_'+mhpostid);
            //updateLink.onclick = '';
...
function mod_hsuforum_getXmlHttpObject() {
    var xmlHttp = null;
    try {
...
/**
 * Checks/unchecks all checkboxes for the passed type
 * 
 * @param string    name        Name of element array to select
 * @param bool      currstate   Current checkbox state 
 **/
function mrcertify_checkall(name, currstate) {
	var els = document.getElementsByName(name+'[]');
    for (var n=0; n < els.length; n++) {
    	if (els[n].disabled == false) {        
    	   els[n].checked = currstate;
        }
    }
}
// Some simple JS to display a pdf generating % complete progress bar to user
function mrcertify_change_percent(percent) {
   document.getElementById("percent").innerHTML = percent+"%";
}
// Function hides the progress bar element
function mrcertify_hide_all() { 
	document.getElementById("percent").style.display = "none";
    document.getElementById("generate_message").style.display = "none";
}     
/**
 * MR - Patch: scorm course popup
 *
 * @return boolean
 */
function mod_scorm_openpopup(url,name,options,width,height) {
    ...
    return openpopup(url,name,options,fullscreen);
}
var responseSuccess = function(o) {

    var response = o.responseText.split('!##$$##!');
    var vnpId = response[0].replace(/[^0-9]/g, '');

    var tbody =  document.getElementById('mod-videonotepad-viewarea'+vnpId).getElementsByTagName('tbody')[0];
    var tr = document.createElement('tr');

    var td1 = document.createElement('td');
    td1.setAttribute("class", "mod-videonotepad notetd");
    td1.appendChild(document.createTextNode(response[1]));

    var td2 = document.createElement('td');
    td2.setAttribute("class", "mod-videonotepad timetd");
    td2.appendChild(document.createTextNode(response[2]));

    tr.appendChild(td1);
    tr.appendChild(td2);
    tbody.appendChild(tr);

    document.getElementById('mod-videonotepad-addarea'+vnpId).value = '';
    document.getElementById('mod-videonotepad-addarea'+vnpId).focus();

    document.getElementById('mod-videonotepad-scrollarea'+vnpId).scrollTop = 99999999;
}
...
function modVideoNotepadAddNote(sURL, vnpId) {
    if (document.getElementById('mod-videonotepad-addarea'+vnpId).value.length != 0) {
        var postData = '';

        if (typeof(modJWPlayerTime) != 'undefined') {
            postData = 'videotime='+Math.round(modJWPlayerTime.getCurrentTime());
        }
        var formObject = document.getElementById('mod-videonotepad-editform'+vnpId);
        YAHOO.util.Connect.setForm(formObject);
        var trans = YAHOO.util.Connect.asyncRequest('POST', sURL, responseHandler, postData);
    }
}/*
ASCIIMathML.js
==============
...
var checkforprocessasciimathinmoodle = true; // true for systems like Moodle
 ...
 * Assignment Form
 **/
blockHierarchy.assignForm = {
    /**
     * Hide/Show second menu
     **/
    actionChange: function() {
        action = document.getElementById('menuaction');
        hierarchy = document.getElementById('menuhierarchyid');

        if (action.options[action.selectedIndex].value == 'moveusers') {
            hierarchy.className = hierarchy.className.replace(new RegExp(' ?hidden'), '');
        } else if (hierarchy.className.indexOf('hidden') == -1) {
            hierarchy.className += ' hidden';
        }
    }
};

/**
 * Block Notify Namespace
 **/
blockNotify = {};
...
blockNotify.newForm = {
    ...
    init: function(options) {
        blockNotify.newForm.options = options;
        document.getElementById('menuthreshold').setAttribute('style', 'display: none');
    },

    /**
     * Populate and display second menu
     **/
    selectChange: function() {
        // Our menus
        var menutype      = document.getElementById('menutype');
        var menuthreshold = document.getElementById('menuthreshold');

        var index = menutype.options[menutype.selectedIndex].value

        if (index == 0) {
            // Hide menu
            menuthreshold.setAttribute('style', 'display: none');
        } else {
            // Clear second menu
            for(var i = menuthreshold.options.length-1; i >= 0; i--) {
                menuthreshold.remove(i);
            }

            // Get our new menu options
            var options = blockNotify.newForm.options[index];
            ...
            for (var i in options) {
                ...
                menuthreshold.appendChild(option);
            }

            // Display menu
            menuthreshold.removeAttribute('style');
        }
    }
};
